...
export function StationRow({ station }: { station: RadioStation }) {
  ...
  return (
    ...
    >
      {/* Icon */}
      <div className="w-12 h-12 rounded-lg overflow-hidden flex-shrink-0 bg-white/4 ring-1 ring-white/6 flex items-center justify-center">
        {station.favicon ? (
          <img
            src={station.favicon}
            alt=""
        ...
        <div className={`flex items-center justify-center text-accent/40 ${station.favicon ? 'hidden' : ''}`}>
      ...
      >
        <svg className={`w-[18px] h-[18px] ${fav ? 'text-red-400 fill-red-400' : 'text-muted'}`} fill={fav ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>

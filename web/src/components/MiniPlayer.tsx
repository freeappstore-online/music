...
export function MiniPlayer() {
  ...
  return (
    <>
      ...
      <div className="md:hidden fixed bottom-14 left-0 right-0 z-40 px-2" onClick={() => setShowFull(true)}>
        <div className="max-w-2xl mx-auto relative">
          ...
          <div className="absolute top-0 left-0 right-0 h-[2px] bg-white/5 rounded-full overflow-hidden z-10">
            <div className="h-full bg-gradient-to-r from-[var(--accent)] to-[var(--accent)]/60 transition-all duration-500" style={{ width: `${progress * 100}%` }} />
          </div>
      ...
      {/* ===== DESKTOP bottom bar ===== */}
      <div className="hidden md:block fixed bottom-0 left-56 right-0 z-40 bg-[var(--surface)]/95 backdrop-blur-xl border-t border-white/[0.06]">
        ...
        <div className="flex items-center gap-4 px-6 py-3 max-w-6xl mx-auto">
          ...
          <div className="flex-1 flex flex-col items-center gap-1.5">
            <div className="flex items-center gap-4">
              {ps.track && (
                <button onClick={() => player.previous()} className="p-1.5 rounded-full hover:bg-white/[0.06] text-[var(--text-muted)] hover:text-[var(--text)] transition-colors">
              ...
              {ps.track && (
                <button onClick={() => player.next()} className="p-1.5 rounded-full hover:bg-white/[0.06] text-[var(--text-muted)] hover:text-[var(--text)] transition-colors">
            ...
            {ps.track && ps.duration > 0 && (
              <div className="flex items-center gap-2 w-full max-w-md">
                <span className="text-[10px] text-[var(--text-muted)] w-8 text-right tabular-nums">{fmt(ps.currentTime)}</span>
                <input
                  type="range" min={0} max={1} step={0.001} value={progress}
                  onChange={(e) => player.seek(parseFloat(e.target.value))}
                  className="flex-1"
                />
                <span className="text-[10px] text-[var(--text-muted)] w-8 tabular-nums">{fmt(ps.duration)}</span>
              </div>
            )}
          </div>
...
function Artwork({ src, size }: { src?: string; size: number }) {
  return (
    <div className="rounded-lg overflow-hidden flex-shrink-0 bg-white/[0.04] ring-1 ring-white/[0.06]" style={{ width: size, height: size }}>
      {src ? (
...
function PlayPauseBtn() {
  ...
  return (
    <button className="w-9 h-9 flex items-center justify-center rounded-full bg-white/[0.06] hover:bg-white/[0.1]" onClick={(e) => { e.stopPropagation(); player.togglePlayPause() }}>
      {ps.isPlaying ? (
...
function NextBtn() {
  return (
    <button className="w-9 h-9 flex items-center justify-center rounded-full bg-white/[0.06] hover:bg-white/[0.1]" onClick={(e) => { e.stopPropagation(); player.next() }}>
